-- =====================================================
-- Bpicius — Fix vendor + customer signup RLS
-- Run in Supabase SQL Editor when you see:
--   "new row violates row-level security policy for table vendors/users"
-- Safe to re-run.
-- =====================================================

-- -----------------------------------------------------------------
-- 1. Vendor SELECT — approved public, own row, admin all
-- -----------------------------------------------------------------
...
-- -----------------------------------------------------------------
-- 2. Vendor INSERT — authenticated signup + admin
-- -----------------------------------------------------------------
...
-- -----------------------------------------------------------------
-- 3. Vendor UPDATE — admin + storefront owner (from TIER_SYSTEM)
-- -----------------------------------------------------------------
...
-- -----------------------------------------------------------------
-- 4. Users — allow role promotion to vendor on own row (signup flow)
-- -----------------------------------------------------------------
...
-- -----------------------------------------------------------------
-- 5. RPC — vendor application (bypasses RLS safely)
-- -----------------------------------------------------------------
...
BEGIN
  ...
  VALUES (
    ...
    coalesce(nullif(trim(p_cuisine), ''), 'Local vendor'),
    coalesce(nullif(trim(p_cuisine), ''), 'Local vendor application'),
    v_email,
    'pending',
...
-- -----------------------------------------------------------------
-- 6. RPC — admin creates approved vendor (requires Supabase admin session)
-- -----------------------------------------------------------------
...
BEGIN
  IF NOT public.is_admin() THEN
    RAISE EXCEPTION 'Admin access required. Sign in with MKJR21@bpicius.com email + password (Supabase Auth), not Auth0-only.';
  END IF;
...
-- -----------------------------------------------------------------
-- 7. RPC — customer signup (bypasses RLS safely)
-- -----------------------------------------------------------------
