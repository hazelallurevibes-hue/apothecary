...
export const PRODUCE_LISTING_CATEGORIES = WELLNESS_PRODUCT_CATEGORIES.map((c) => ({
  ...c,
...
export function allFarmersMarketCategories() {
...
export function getFarmersMarketCategoryLabel(id) {
  return getProductCategoryLabel(id);
}
...
export function getCategoryDisplay(categoryId) {
  const label = getFarmersMarketCategoryLabel(categoryId);
