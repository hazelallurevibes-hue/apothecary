-- =====================================================
-- Hazel Allure — Vendor listing INSERT + RPC fallback
-- Run in Supabase SQL Editor after FIX_VENDOR_LISTING_CRUD.sql
-- Safe to re-run.
-- =====================================================
...
BEGIN
  ...
  SET
    ...
    price = v_rec.price,
    description = v_rec.description,
    availability = COALESCE(NULLIF(v_rec.availability, ''), availability, 'In stock'),
    ...
    item_options = v_rec.item_options,
    last_activity_at = COALESCE(v_rec.last_activity_at, NOW()),
    fulfillment_mode = COALESCE(v_rec.fulfillment_mode, fulfillment_mode, 'pickup_and_shipping'),
    service_video_url = v_rec.service_video_url,
    service_video_provider = v_rec.service_video_provider,
    media_type = COALESCE(v_rec.media_type, media_type),
    gallery_photos = v_rec.gallery_photos
